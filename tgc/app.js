...
function startGame() {
    playerText.textContent = `${player} Başla !`
...
function chooseArea(block) {
    if (block.textContent === "") {
        ...
        errorText.textContent = "Farklı Seçim Yapınız ! "
    ...
    if (gameOver) {
        playerText.textContent = `Oyun Bitti, ${winner} Tebrikler Kazandınız ! `;
...
function turnPlayer() {
    if (player === "X") {
        player = "O";
        playerText.textContent = `${player} Sıra Sende !`
        return;
    } else if (player === "O") {
        player = "X";
        playerText.textContent = `${player} Sıra Sende !`
